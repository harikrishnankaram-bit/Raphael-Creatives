...
function MenuOverlay({ onClose }: { onClose: () => void }) {
  return (
    ...
    >
      <div className="flex min-h-screen flex-col justify-between px-6 py-24 md:px-10 md:py-32 relative">
        {/* Main Nav items */}
        <div className="flex flex-col items-center justify-center my-auto">
            ...
            {primaryNav.map((item, i) => (
              ...
              >
                <Link
                  href={item.path}
                  onClick={onClose}
                  className="text-4xl font-bold leading-[1.1] text-foreground transition-opacity hover:opacity-60 md:text-8xl block py-1"
                >
                  {item.name}
                </Link>
              </motion.div>
          ...
          <motion.div
            ...
            className="mt-8 flex items-center justify-center gap-8 text-muted-foreground"
          >
        ...
        >
          ...
          <Link
            href="/main-services"
            onClick={onClose}
            className="flex w-full items-center justify-between gap-6 rounded-xl border border-border bg-secondary/40 p-4 backdrop-blur-md transition-colors hover:bg-secondary/70 md:w-96"
          >
